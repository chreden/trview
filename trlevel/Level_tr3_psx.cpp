...
namespace trlevel
{
    namespace
    {
        ...
        std::vector<uint8_t> convert_vag_to_wav(const std::vector<uint8_t>& bytes)
        {
            ...
            uint32_t sample_frequency = 11025;
    ...
    void Level::load_tr3_psx(std::basic_ispanstream<uint8_t>& file, trview::Activity& activity, const LoadCallbacks& callbacks)
    {
        ...
        for (uint32_t s = 0; s < sound_offsets.size(); ++s)
        {
            ...
            size;
            const auto converted = convert_vag_to_wav({ &sound_data[offset], &sound_data[offset + size] });

            // TODO: Remove temporary dump
            std::ofstream out_file;
            out_file.open(std::format("C:\\dev\\trview-temp\\sounds\\{}.wav", s), std::ios::binary | std::ios::out);
            out_file.write(reinterpret_cast<const char*>(&converted[0]), converted.size());
            out_file.close();
        ...
        // TR3:
        // Num samples(uint32) at offset 0x04, followed by an array of raw sample offsets(uint32[num samples]) and the combined sample size(in bytes).
        // Raw sample data then follows consecutively.
        // Note: This format also applies to the TR3 ECTS demo and later beta builds.
