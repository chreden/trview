...
namespace trview
{
    ...
    void Sector::generate_ceiling_steps(const Portal& self, const Portal& north, const Portal& east, const Portal& south, const Portal& west)
    {
        ...
        {
            add_quad_remote(self, east, Quad(east.ceiling(Corner::SW), ceiling(Corner::NE), ceiling(Corner::SE), east.ceiling(Corner::NW), wall_flags & (~SectorFlag::Climbable | SectorFlag::ClimbableWest), _room));
        }

        if (west && (ceiling(Corner::NW) != west.ceiling(Corner::NE) || ceiling(Corner::SW) != west.ceiling(Corner::SE)))
        {
            add_quad_remote(self, west, Quad(ceiling(Corner::SW), west.ceiling(Corner::NE), west.ceiling(Corner::SE), ceiling(Corner::NW), wall_flags & (~SectorFlag::Climbable | SectorFlag::ClimbableEast), _room));
        }
    }
