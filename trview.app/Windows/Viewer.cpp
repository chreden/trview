...
namespace trview
{
    namespace
    {
        ...
        template <typename T>
        std::shared_ptr<T> get_element(const PickResult& result)
        {
            return std::static_pointer_cast<T>(result.element.lock());
        }
        ...
        std::string generate_pick_message(const PickResult& result, const ILevel& level)
        {
            ...
            switch (result.type)
            {
                case PickResult::Type::Entity:
                {
                    if (const auto item = get_element<IItem>(result))
                    {
                ...
                case PickResult::Type::Trigger:
                {
                    if (const auto trigger = get_element<ITrigger>(result))
                    {
                ...
                case PickResult::Type::Light:
                {
                    if (const auto light = get_element<ILight>(result))
                    {
                ...
                {
                    if (const auto room = get_element<IRoom>(result))
                    {
                ...
                {
                    if (const auto waypoint = get_element<IWaypoint>(result))
                    {
                ...
                case PickResult::Type::CameraSink:
                {
                    if (const auto camera_sink = get_element<ICameraSink>(result))
                    {
                ...
                {
                    if (const auto static_mesh = get_element<IStaticMesh>(result))
                    {
                ...
                {
                    if (const auto scriptable = get_element<IScriptable>(result))
                    {
                ...
                {
                    if (const auto sound_source = get_element<ISoundSource>(result))
                    {
                ...
                {
                    if (const auto flyby_node = get_element<IFlybyNode>(result))
                    {
        ...
        _clipboard(clipboard), _camera(camera), _sampler_source(sampler_source), _messaging(messaging)
    {
        ...
        _token_store += _ui->on_add_waypoint += [&]()
        {
            ...
            switch (_context_pick.type)
            {
                case PickResult::Type::Room:
                {
                    if (const auto room = get_element<IRoom>(_context_pick))
                    {
                        index = room->number();
                    }
                    break;
                ...
                case PickResult::Type::Entity:
                {
                    if (const auto item = get_element<IItem>(_context_pick))
                    {
                ...
                case PickResult::Type::Trigger:
                {
                    if (const auto trigger = get_element<ITrigger>(_context_pick))
                    {
                ...
                case PickResult::Type::Light:
                {
                    if (const auto light = get_element<ILight>(_context_pick))
                    {
                ...
                case PickResult::Type::CameraSink:
                {
                    if (const auto camera_sink = get_element<ICameraSink>(_context_pick))
                    {
        ...
        _token_store += _ui->on_hide += [&]()
        {
            if (auto level = _level.lock())
            {
                ...
                {
                    if (auto item = get_element<IItem>(_context_pick))
                    {
                ...
                {
                    if (auto trigger = get_element<ITrigger>(_context_pick))
                    {
                ...
                {
                    if (auto light = get_element<ILight>(_context_pick))
                    {
                ...
                {
                    if (auto room = get_element<IRoom>(_context_pick))
                    {
                ...
                {
                    if (auto camera_sink = get_element<ICameraSink>(_context_pick))
                    {
                ...
                {
                    if (auto mesh = get_element<IStaticMesh>(_context_pick))
                    {
                ...
                {
                    if (auto sound_source = get_element<ISoundSource>(_context_pick))
                    {
                    ...
                    auto get_number_for_pick = [&](auto&& pick) -> uint32_t
                        {
                            ...
                            case PickResult::Type::Room:
                                return get_number(get_element<IRoom>(pick));
                            case PickResult::Type::Entity:
                                return get_number(get_element<IItem>(pick));
                            case PickResult::Type::Trigger:
                                return get_number(get_element<ITrigger>(pick));
                            case PickResult::Type::StaticMesh:
                                return get_number(get_element<IStaticMesh>(pick));
                            case PickResult::Type::Light:
                                return get_number(get_element<ILight>(pick));
                            case PickResult::Type::CameraSink:
                                return get_number(get_element<ICameraSink>(pick));
                            case PickResult::Type::SoundSource:
                                return get_number(get_element<ISoundSource>(pick));
        ...
        _token_store += _ui->on_filter_items_to_tile += [&](auto&& window_ptr)
            {
                ...
                if (auto window = window_ptr.lock())
                {
                    ...
                    const auto get_room = [&](auto&& ent)
                        {
                            if (ent)
                            {
                                if (const auto room = ent->room().lock())
                                {
                    ...
                    switch (_context_pick.type)
                    {
                        ...
                        case PickResult::Type::Entity:
                            get_room(get_element<IItem>(_context_pick));
                            break;
                        case PickResult::Type::Trigger:
                            get_room(get_element<ITrigger>(_context_pick));
                            break;
                        case PickResult::Type::Light:
                            get_room(get_element<ILight>(_context_pick));
                            break;
                        case PickResult::Type::CameraSink:
                            get_room(get_element<ICameraSink>(_context_pick));
                            break;
                    }
                }
            };
        ...
        _token_store += _picking->on_pick += [&](PickInfo, PickResult result)
        {
            ...
            if (level)
            {
                ...
                if (result.hit)
                {
                    ...
                    {
                        const auto room = get_element<IRoom>(_current_pick);
                    ...
                    {
                        const auto trigger = get_element<ITrigger>(_current_pick);
    ...
    void Viewer::initialise_input()
    {
        ...
        _token_store += _mouse->mouse_click += [&](auto button)
        {
            if (button == input::IMouse::Button::Right && _current_pick.hit && _current_pick.type != PickResult::Type::Compass && _camera->idle_rotation())
            {
                ...
                {
                    const auto item = get_element<IItem>(_current_pick);
                ...
                {
                    const auto camera_sink = get_element<ICameraSink>(_current_pick);
    ...
    std::weak_ptr<IRoom> Viewer::room_from_pick(const PickResult& pick) const
    {
        ...
        switch (pick.type)
        {
            case PickResult::Type::Room:
            {
                return get_element<IRoom>(pick);
            }
            case PickResult::Type::Entity:
            {
                if (auto item = get_element<IItem>(pick))
                {
                    return item->room();
                }
                break;
            ...
            case PickResult::Type::Trigger:
            {
                if (const auto trigger = get_element<ITrigger>(pick))
                {
                    return trigger->room();
                }
                break;
            ...
            case PickResult::Type::Waypoint:
            {
                if (const auto waypoint = get_element<IWaypoint>(pick))
                {
            ...
            case PickResult::Type::Light:
            {
                if (const auto light = get_element<ILight>(pick))
                {
                    return light->room();
                }
                break;
            ...
            case PickResult::Type::CameraSink:
            {
                if (const auto camera_sink = get_element<ICameraSink>(pick))
                {
    ...
    void Viewer::select_pick(const PickResult& pick)
    {
        switch (pick.type)
        {
            case PickResult::Type::Room:
            {
                messages::send_select_room(_messaging, get_element<IRoom>(pick));
            ...
            case PickResult::Type::Entity:
            {
                messages::send_select_item(_messaging, get_element<IItem>(pick));
                break;
            }
            case PickResult::Type::Trigger:
            {
                messages::send_select_trigger(_messaging, get_element<ITrigger>(pick));
                break;
            }
            case PickResult::Type::Waypoint:
            {
                messages::send_select_waypoint(_messaging, get_element<IWaypoint>(pick));
                break;
            }
            case PickResult::Type::Light:
            {
                messages::send_select_light(_messaging, get_element<ILight>(pick));
                break;
            }
            case PickResult::Type::CameraSink:
            {
                messages::send_select_camera_sink(_messaging, get_element<ICameraSink>(pick));
                break;
            }
            case PickResult::Type::StaticMesh:
            {
                messages::send_select_static_mesh(_messaging, get_element<IStaticMesh>(pick));
                break;
            }
            case PickResult::Type::Scriptable:
            {
                if (auto scriptable = get_element<IScriptable>(pick))
                {
                    scriptable->click();
                }
                break;
            ...
            case PickResult::Type::SoundSource:
            {
                messages::send_select_sound_source(_messaging, get_element<ISoundSource>(pick));
                break;
            }
            case PickResult::Type::FlybyNode:
            {
                messages::send_select_flyby_node(_messaging, get_element<IFlybyNode>(pick));
                break;
            }
        }
    }
