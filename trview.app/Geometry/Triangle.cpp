...
namespace trview
{
    ...
    bool determine_transparency(uint16_t attribute, uint16_t effects, Triangle::TransparencyMode& out)
    {
        ...
        if (effects & 0x1)
        {
            out = Triangle::TransparencyMode::Additive;
            return true;
        }

        if (!attribute)
        {
            out = Triangle::TransparencyMode::None;
            return false;
        }

        out = attribute == 2 ? Triangle::TransparencyMode::Additive : Triangle::TransparencyMode::Normal;
        return true;
    }
}
