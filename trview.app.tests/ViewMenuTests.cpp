...
{
    ViewMenu menu(create_menu_window(ID_APP_VIEW_MINIMAP));

    std::optional<bool> raised_value;
    auto token = menu.on_show_minimap += [&raised_value](bool value) { raised_value = value; };

    menu.process_message(WM_COMMAND, MAKEWPARAM(ID_APP_VIEW_MINIMAP, 0), 0);
...
{
    ViewMenu menu(create_menu_window(ID_APP_VIEW_TOOLTIP));

    std::optional<bool> raised_value;
    auto token = menu.on_show_tooltip += [&raised_value](bool value) { raised_value = value; };

    menu.process_message(WM_COMMAND, MAKEWPARAM(ID_APP_VIEW_TOOLTIP, 0), 0);
    ASSERT_TRUE(raised_value.has_value());
    ASSERT_FALSE(raised_value.value());
...
{
    ViewMenu menu(create_menu_window(ID_APP_VIEW_UI));

    std::optional<bool> raised_value;
    auto token = menu.on_show_ui += [&raised_value](bool value) { raised_value = value; };

    menu.process_message(WM_COMMAND, MAKEWPARAM(ID_APP_VIEW_UI, 0), 0);
    ASSERT_TRUE(raised_value.has_value());
    ASSERT_FALSE(raised_value.value());
...
{
    ViewMenu menu(create_menu_window(ID_APP_VIEW_SELECTION));

    std::optional<bool> raised_value;
    auto token = menu.on_show_selection += [&raised_value](bool value) { raised_value = value; };

    menu.process_message(WM_COMMAND, MAKEWPARAM(ID_APP_VIEW_SELECTION, 0), 0);
    ASSERT_TRUE(raised_value.has_value());
    ASSERT_FALSE(raised_value.value());
...
{
    ViewMenu menu(create_menu_window(ID_APP_VIEW_ROUTE));

    std::optional<bool> raised_value;
    auto token = menu.on_show_route += [&raised_value](bool value) { raised_value = value; };

    menu.process_message(WM_COMMAND, MAKEWPARAM(ID_APP_VIEW_ROUTE, 0), 0);
    ASSERT_TRUE(raised_value.has_value());
    ASSERT_FALSE(raised_value.value());
...
{
    ViewMenu menu(create_menu_window(ID_APP_VIEW_TOOLS));

    std::optional<bool> raised_value;
    auto token = menu.on_show_tools += [&raised_value](bool value) { raised_value = value; };

    menu.process_message(WM_COMMAND, MAKEWPARAM(ID_APP_VIEW_TOOLS, 0), 0);
    ASSERT_TRUE(raised_value.has_value());
    ASSERT_FALSE(raised_value.value());
...
{
    ViewMenu menu(create_menu_window(ID_APP_VIEW_UNHIDE_ALL));

    bool raised;
    auto token = menu.on_unhide_all += [&raised]() { raised = true; };

    menu.process_message(WM_COMMAND, MAKEWPARAM(ID_APP_VIEW_UNHIDE_ALL, 0), 0);
    ASSERT_TRUE(raised);
}
