    ...
    auto register_test_module()
    {
        struct test_module
        {
            ...
            std::unique_ptr<ViewerUI> build()
            {
                ...
                return std::make_unique<ViewerUI>(window, shortcuts, map_renderer_source, std::move(settings_window), std::move(view_options), std::move(context_menu), std::move(camera_controls), std::move(toolbar), messaging, main_menu);
            }
