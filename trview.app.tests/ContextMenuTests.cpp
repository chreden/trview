...
TEST(ContextMenu, RandoLocationNotPresentWhenDisabled)
{
    ui::Window parent(Size(800, 600), Colour::Transparent);
    ContextMenu menu(parent);

    auto button = parent.find<ui::Button>(ContextMenu::Names::rando_location_button);
    ASSERT_EQ(button, nullptr);
}

TEST(ContextMenu, RandoLocationRaised)
{
    ui::Window parent(Size(800, 600), Colour::Transparent);
    ContextMenu menu(parent);
    menu.set_visible(true);
    menu.set_randomizer_tools(true);

    bool raised = false;
    auto token = menu.on_rando_location += [&raised]() { raised = true; };
...
TEST(ContextMenu, RandoLocationRemovedWhenDisabled)
{
    ui::Window parent(Size(800, 600), Colour::Transparent);
    ContextMenu menu(parent);
    menu.set_randomizer_tools(true);
    ...
    menu.set_randomizer_tools(false);
    button = parent.find<ui::Button>(ContextMenu::Names::rando_location_button);
    ASSERT_EQ(button, nullptr);
}
