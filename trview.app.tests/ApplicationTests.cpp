...
#include <trview.app/Mocks/Windows/ILauWindowManager.h>
...
namespace
{
    ...
    auto register_test_module()
    {
        struct test_module
        {
            ...
            std::unique_ptr<ILauWindowManager> lau_window_manager{ std::make_unique<MockLauWindowManager>() };
            ...
            std::unique_ptr<Application> build()
            {
                ...
                return std::make_unique<Application>(window, std::move(update_checker), std::move(settings_loader), std::move(file_dropper),
                    ...
                    level_source, startup_options, dialogs, files, std::move(lau_window_manager));
            }
