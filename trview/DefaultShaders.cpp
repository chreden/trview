...
namespace trview
{
    namespace
    {
        void load_level_shaders(const CComPtr<ID3D11Device>& device, IShaderStorage& storage)
        {
            ...
            storage.add("level_vertex_shader", std::make_unique<VertexShader>(device, load_file(L"level_vertex_shader.cso"), input_desc));
            storage.add("level_pixel_shader", std::make_unique<PixelShader>(device, load_file(L"level_pixel_shader.cso")));
        }

        void load_ui_shaders(const CComPtr<ID3D11Device>& device, IShaderStorage& storage)
        {
            ...
            storage.add("ui_vertex_shader", std::make_unique<VertexShader>(device, load_file(L"ui_vertex_shader.cso"), input_desc));
            storage.add("ui_pixel_shader", std::make_unique<PixelShader>(device, load_file(L"ui_pixel_shader.cso")));
        }
    }
