...
namespace trview
{
    ...
    LevelInfo::LevelInfo(ui::Control& control, const ITextureStorage& texture_storage)
    {
        ...
        {
            _panel->set_position(Point(std::roundf(size.width / 2.0f - _panel->size().width / 2.0f), 0));
        };
        _panel->on_size_changed += [&](const Size& size)
        {
            auto parent = _panel->parent();
            _panel->set_position(Point(std::roundf(parent->size().width / 2.0f - size.width / 2.0f), 0));
        };
    }
