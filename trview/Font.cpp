...
namespace trview
{
    ...
    FontTexture Font::create_texture(Texture& texture)
    {
        CComPtr<IDXGISurface> surface;
        texture.texture->QueryInterface(&surface);
    ...
    FontTexture Font::create_texture()
    {
        ...
        _device->CreateTexture2D(&desc, nullptr, &new_texture.texture.texture);

        CComPtr<IDXGISurface> surface;
        new_texture.texture.texture->QueryInterface(&surface);
        ...
        _d2d_factory->CreateDxgiSurfaceRenderTarget(surface, &props, &new_texture.render_target);
        new_texture.render_target->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White), &new_texture.brush);
        _device->CreateShaderResourceView(new_texture.texture.texture, nullptr, &new_texture.texture.view);
        return new_texture;
    }
