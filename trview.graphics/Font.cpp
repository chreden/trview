...
namespace trview
{
    namespace graphics
    {
        ...
        void Font::render(const Microsoft::WRL::ComPtr<ID3D11DeviceContext>& context, const std::wstring& text, float x, float y, float width, float height, const Colour& colour)
        {
            ...
            _font->DrawString(_batch.get(), sanitise(*_font, text).c_str(), XMVectorSet(round(x), round(y), 0, 0), XMVectorSet(colour.r, colour.g, colour.b, colour.a), 0, XMVectorZero(), XMVectorSet(1, 1, 1, 1), SpriteEffects_None, 0.0f);
            _batch->End();
        }
